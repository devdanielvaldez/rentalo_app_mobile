...
module.exports = (req, res) => {
  ...
  const userPromise = sdk.users.show({ id: bookingData.authorId });

  Promise.all([listingPromise, userPromise])
    .then(([apiResponse, userResponse]) => {
      const listing = apiResponse.data.data;
      const provider = userResponse.data.data;
      const lineItems = transactionLineItems(listing, bookingData, provider);
