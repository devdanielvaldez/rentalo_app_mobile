...
const publicKey = process.env.SENDGRID_VERIFICATION_KEY;
// ('MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEaaW62/liOpSRjjEtUPYz+unV6omyp442K3Tskj2z4UorWUw75By2PtQF4ZCA3uQYwN7XeS5GbmD3tIh4CsLwXQ=');
...
app.post('/sendgrid/webhook', async (req, res) => {
  try {
    ...
    const isValid = webhook.verifySignature(ecPublicKey, payload, signature, timestamp);
    // if (!isValid) {
    //   return res.status(401).json({ error: 'Unauthorized' });
    // }
    console.log('Email event received', payload);
    const isDelivered = payload?.find(event => event.event === 'delivered');
    if (isDelivered) {
      const { listingId, userId } = isDelivered;
      if (listingId && userId) {
        console.log('Email delivered for user', userId);
        console.log('Email delivered for listing', listingId);
        const scheduleNewReminder = await createScheduleEmail(userId, listingId);
        console.log('New schedule email created for user', userId);
      }
    }

    res.status(200).send('OK');
  } catch (e) {
    console.log('Error in sendgrid webhook', e);
  }
});
...
app.use(function(req, res, next) {
  ...
  res.setHeader('Access-Control-Allow-Origin', 'https://www.rentaloinc.com');
