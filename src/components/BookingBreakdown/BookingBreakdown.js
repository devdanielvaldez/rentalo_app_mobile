...
import { convertTo12HourFormat } from '../../util/dates';
...
import LineItemExtendTrip from './LineItemExtendTrip';
...
export const BookingBreakdownComponent = props => {
  ...
  const { pickUpTime, dropTime } = bookingData || {};
  ...
  const extendedBookingData = transaction?.attributes?.protectedData?.extendedBookingData?.length;

  const exceedsCharge = transaction?.attributes?.metadata?.dropOffExceedCharge;
  const payinTotal = transaction.attributes.payinTotal;

  const pickupTime = pickUpTime && convertTo12HourFormat(pickUpTime);
  ...
  return (
    <div className={classes}>
      ...
      {isExtend && extendedBookingData && (
        <div className={css.bookingExtended}>
          <h2>Extended booking </h2>
          <LineItemExtendTrip
            transaction={transaction}
            isProvider={isProvider}
            isCustomer={isCustomer}
      ...
      {isCustomer && exceedsCharge ? (
        <div className={css.exceedsCharge}>
          <FormattedMessage id="BookingBreakdown.exceedsCharge" />
          <span>{formatMoney(intl, new Money(exceedsCharge.amount, payinTotal.currency))}</span>
        </div>
      ) : null}
    </div>
  );
