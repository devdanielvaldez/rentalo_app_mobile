  ...
  constructor(props) {
    ...
    try {
      ...
      const formattedValue = hasInitialValue
        ? intl.formatNumber(ensureDotSeparator(unformattedValue), currencyConfig).replaceAll(',', '.')
        : '';
  ...
  updateValues(event) {
    try {
      ...
      const formattedValue = !isEmptyString
        ? intl.formatNumber(ensureDotSeparator(truncatedValueString), currencyConfig).replaceAll(',', '.')
        : '';
