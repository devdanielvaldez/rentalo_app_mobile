...
export const convertTo12HourFormat = time => {
  ...
  if (hours === 0) {
    ...
    return '12:' + (minutes < 10 ? '0' : '') + minutes + ' PM';
  } else if (hours > 12) {
    hours -= 12;
...
export const combineDates = (date, time) => {
  date = new Date(date);
  const [timePart, period] = time.split(' '); // "4:00" and "AM"
  let [hours] = timePart.split(':'); // hours = "4", minutes = "00"

  // Convert to 24-hour format if needed
  hours = period === 'PM' && hours !== '12' ? parseInt(hours, 10) + 12 : hours;
  hours = period === 'AM' && hours === '12' ? '00' : hours; // Handle midnight

  // Set the new hours and minutes on the existing Date object
