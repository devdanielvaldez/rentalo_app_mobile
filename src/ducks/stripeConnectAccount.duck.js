...
export const STRIPE_RECIPIENT_ACCOUNT_FETCH_REQUEST =
  'app/stripe/STRIPE_RECIPIENT_ACCOUNT_FETCH_REQUEST';
export const STRIPE_RECIPIENT_ACCOUNT_FETCH_SUCCESS =
  'app/stripe/STRIPE_RECIPIENT_ACCOUNT_FETCH_SUCCESS';
export const STRIPE_RECIPIENT_ACCOUNT_FETCH_ERROR =
  'app/stripe/STRIPE_RECIPIENT_ACCOUNT_FETCH_ERROR';
...
export default function reducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case STRIPE_RECIPIENT_ACCOUNT_FETCH_ERROR:
      console.error(payload);
...
export const stripeRecipientAccountFetchRequest = () => ({
  type: STRIPE_RECIPIENT_ACCOUNT_FETCH_REQUEST,
});
export const stripeRecipientAccountFetchSuccess = stripeAccount => ({
  type: STRIPE_RECIPIENT_ACCOUNT_FETCH_SUCCESS,
  payload: stripeAccount,
});
export const stripeRecipientAccountFetchError = e => ({
  type: STRIPE_RECIPIENT_ACCOUNT_FETCH_ERROR,
  payload: e,
  error: true,
});
...
export const createStripeAccount = params => (dispatch, getState) => {
  ...
  const userId = user?.currentUser?.id?.uuid;
  ...
  return stripe
    ...
    .then(response => {
      ...
      return createRecipientAccount(recipientParams);
    })
    .then(response => {
      // const stripeAccount = response.data.data;
      const stripeAccount = response;
...
export const updateStripeAccount = params => (dispatch, getState, sdk) => {
  ...
  //     { expand: true }
  //   )
  return updateRecipientAccount(params)
    .then(response => {
      const stripeAccount = response.data.data;
...
export const getStripeConnectAccountLink = params => dispatch => {
  const { failureURL, successURL, type, accountId } = params;
  dispatch(getAccountLinkRequest());

  const stripeRecipientAccountLinksParams = {
    accountId,
    refreshUrl: failureURL,
  ...
  //   })
  return createRecipientAccountLink(stripeRecipientAccountLinksParams)
    .then(response => {
...
export const fetchRecipientStripeAccount = id => dispatch => {
  dispatch(stripeRecipientAccountFetchRequest());

  return getRecipientAccount({ id })
    .then(response => {
      dispatch(stripeRecipientAccountFetchSuccess(response));
      return response;
    })
    .catch(err => {
      const e = storableError(err);
      dispatch(stripeRecipientAccountFetchError(e));
      const stripeMessage =
        e.apiErrors && e.apiErrors.length > 0 && e.apiErrors[0].meta
          ? e.apiErrors[0].meta.stripeMessage
          : null;
