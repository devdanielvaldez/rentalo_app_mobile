...
export const CREATE_CUSTOMER_REQUEST = 'app/user/CREATE_CUSTOMER_REQUEST';
export const CREATE_CUSTOMER_SUCCESS = 'app/user/CREATE_CUSTOMER_SUCCESS';
...
export default function reducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case CREATE_CUSTOMER_REQUEST:
      return { ...state, attachmentInProgress: true };
    case CREATE_CUSTOMER_SUCCESS:
      return { ...state, attachmentInProgress: false, attached: true };
  ...
  return sdk.ownListings
    ...
    .then(response => {
      ...
      const hasPublishedListing =
        response?.data?.data &&
        response?.data?.data?.find(l => l?.attributes?.state !== LISTING_STATE_DRAFT);
      const hasPublishedListings = hasListings && ensureOwnListing(hasPublishedListing);
...
export const createCustomer = params => async (dispatch, getState) => {
  try {
    dispatch(createCustomerRequest());
    const currentUser = getState().user.currentUser;
    const email = currentUser?.attributes?.email;
    const customer = await createStripeCustomer({ ...params, email });
...
export const getCountryId = params => async () => {
  try {
    const countryNumber = await getOdooCountryCode({ countryName: params });
...
export const updateOdooUser = params => async () => {
  try {
    const res = await updateOdooUsers({ ...params });
    return res;
  } catch (error) {
    console.log(error);
  }
...
export const fetchPaymentMethod = customerId => async () => {
  const paymentMethod = await retrievePaymentMethod(customerId);
  return paymentMethod && paymentMethod.hasCard;
};

export const createIntent = params => async () => {
  try {
    // return console.log('parametros ---->', params);
    // const { payment, paymentIntent } = await createPaymentIntent({ ...params });
    // return console.log('paymentIntent ---->', paymentIntent)
    return {
      payment: 'SUCCESS',
...
export const sendMailByChimp = params => () => {
  try {
    return mailChimpApi({ ...params }).then(res => res);
  } catch (error) {
    console.log(error, 'Error while send Mail By Chimp!!');
  }
};
